...
from settings import (
    ...
    TIMEOUT_SECONDS, DATA_ENDPOINT,
    JITTER_MIN, JITTER_MAX, YEAR_BOUNDARY_SLEEP,
...
from src.station_service import create_station_url, get_headers_for_station, find_station_by_code, search_and_print_stations
...
async def capture_post_response(page, response_queue: asyncio.Queue, trigger_action) -> str:
    """
...
async def process_option(page, response_queue: asyncio.Queue, option, csv_manager, save_individual=True) -> bool:
    """
    ...
    async def select_action():
        ...
        if not option_elem:
            raise SelectNotFoundError(f"Opción no encontrada en el select: {option['value']}")
    ...
    for attempt in range(1, MAX_RETRIES + 2):  # intentos: 1..MAX_RETRIES+1
        try:
            ...
            if not data_table:
                raise TableNotFoundError(f"No se encontró table#dataTable en la respuesta del endpoint")

            table_html = str(data_table)
            print(f"{SUCCESS} Datos capturados para {option['value']} ({len(full_html):,} bytes)")
            ...
            if attempt <= MAX_RETRIES:
                print(f"{WARNING} Intento {attempt} fallido para {option['value']}: {e}")
                print(f"{PROCESSING} Reintentando en {RETRY_SLEEP}s...")
                await asyncio.sleep(RETRY_SLEEP)
            else:
                print(f"{ERROR} Error procesando opción {option['value']} tras {attempt} intentos: {e}")
                import traceback
                traceback.print_exc()
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("-y", "--year",  type=int, metavar="AÑO",  help="Año (requerido para month y year).")
    parser.add_argument("--month",       type=int, metavar="MES",  help="Mes 1-12 (solo para --mode month).")
    parser.add_argument("--start",       type=int, metavar="AÑO",  help="Año inicial (solo para --mode period).")
    parser.add_argument("--end",         type=int, metavar="AÑO",  help="Año final   (solo para --mode period).")
...
def validate_cli_args(args: argparse.Namespace) -> None:
    ...
    if mode == "month":
        if args.year is not None and not (2000 <= args.year <= 2050):
            sys.exit(f"{ERROR} --year inválido: {args.year}. Debe estar entre 2000 y 2050.")
        if args.month is not None and not (1 <= args.month <= 12):
            sys.exit(f"{ERROR} --month inválido: {args.month}. Debe estar entre 1 y 12.")
        ...
        if args.year:
            sys.exit(f"{ERROR} --year no aplica para --mode period. Use --start y --end.")
        if args.month:
            sys.exit(f"{ERROR} --month no aplica para --mode period.")
        if args.start is not None and not (2000 <= args.start <= 2050):
            sys.exit(f"{ERROR} --start inválido: {args.start}. Debe estar entre 2000 y 2050.")
        if args.end is not None and not (2000 <= args.end <= 2050):
            sys.exit(f"{ERROR} --end inválido: {args.end}. Debe estar entre 2000 y 2050.")
        if args.start is not None and args.end is not None and args.start > args.end:
            sys.exit(f"{ERROR} --start ({args.start}) no puede ser mayor que --end ({args.end}).")
...
async def main(args: argparse.Namespace = None):
    ...
    if args and args.station:
        ...
        if not query_station:
            sys.exit(f"{ERROR} No se encontró ninguna estación con el código '{args.station}'.\n"
    ...
    cli_dict = {}
    if args:
        if args.mode:   cli_dict["mode"] = args.mode
        if args.year:   cli_dict["year"] = args.year
        if args.month:  cli_dict["month"] = args.month
        if args.start:  cli_dict["start"] = args.start
        if args.end:    cli_dict["end"] = args.end
    ...
    try:
        ...
        csv_manager = CSVManager(station_name, **({} if not csv_output_dir else {"output_dir": csv_output_dir}))
        ...
        for i, option in enumerate(filtered_options, 1):
            ...
            if prev_year is not None and current_year != prev_year:
                print(f"📅 Nuevo año ({current_year}) — pausa de {YEAR_BOUNDARY_SLEEP}s")
                await asyncio.sleep(YEAR_BOUNDARY_SLEEP)
            prev_year = current_year

            success = await process_option(page, response_queue, option, csv_manager, save_individual=save_individual)
            if success:
                successful_count += 1
